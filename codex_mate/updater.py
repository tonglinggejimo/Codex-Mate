...
USER_AGENT = f"Codex Mate/{__version__}"
...
def select_update_asset(assets: list[dict[str, Any]]) -> dict[str, str] | None:
    ...
    for asset in named_assets:
        name = str(asset["name"])
        if name == "CodexMate.zip" or name.lower().startswith("codex_mate-") and name.lower().endswith((".zip", ".tar.gz", ".tgz")):
            return {"name": name, "browser_download_url": str(asset["browser_download_url"])}
    for asset in named_assets:
        name = str(asset["name"]).lower()
        if name.endswith((".zip", ".tar.gz", ".tgz")) and "-windows" not in name and "-macos" not in name:
            return {"name": str(asset["name"]), "browser_download_url": str(asset["browser_download_url"])}
    return None
...
def source_tree_root(module_file: Path = PACKAGE_MODULE_FILE) -> Path | None:
    ...
    if package_dir.name != "codex_mate":
        return None
...
def perform_update(
    ...
    *,
    ...
    if download_dir is None:
        with tempfile.TemporaryDirectory(prefix="codex-mate-update-") as temp_dir:
